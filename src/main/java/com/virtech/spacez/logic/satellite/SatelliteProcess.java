...
public class SatelliteProcess {
	public static Vector<SatelliteLogic> satellites = new Vector<>();

	public static void addSatellite(SatelliteLogic satellite) {
		if (!satellites.contains(satellite)) {
			satellites.add(satellite);
		}
	}

	public static void removeSatellite(SatelliteLogic satellite) {
	...
	static SatelliteRequest makeRequest(Coordinates coordinates, double radius) throws Exception {
		...
		for (int i = (int) Time.current(); i < 43200; i++) {
			for (SatelliteLogic satellite : satellites) {
	...
	private static double getViewRadius(SatelliteLogic satellite, double time) {
		// Round orbit only
